...
`control-rs-xtask` is the host-side orchestration and user interface package for the `control-rs` repository.
...
- It acts as the orchestration framework, invoking cross-compilers (`cargo build --target thumbv7em-none-eabihf`), code linters (`clippy`), test coverages (`tarpaulin`), and launching `qemu-system-arm`.
- It converts raw target bytes into structured Rust `Telemetry` messages and serializes developer controls into `Command` byte streams sent to the target.
...
- `Enter`: Triggers the selected test or toggles the selected configuration setting.
- `q`: Quits the TUI console.

### 2. Running Headless HIL Tests in CI
To execute the automated lints, host-side unit tests, line coverage checks, and headless HIL tests on the QEMU emulator:
```bash
cargo run --package control-rs-xtask -- ci
```
This command generates the `ci-report.md` file summarizing formatted test outputs, clippy errors, coverage percentages, and the detailed cycles and execution times of all target tests.

### 3. Launching QEMU Manually
To build and start the QEMU system emulator target binary directly in standard semihosting console mode, run:
```bash
...
Because `control-rs-xtask` runs on the host system, it can be tested directly with standard cargo commands:

### Host Compilation Check
Ensure the bridge, main command logic, and TUI compile cleanly on the host target:
```bash
...
cargo run --package control-rs-xtask -- ci
```
Verify that `ci-report.md` is successfully generated in the root of the workspace and contains no clippy errors or failed test summaries.
