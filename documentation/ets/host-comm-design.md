...
session management, and decoding are owned by `control-rs-ets-host::ServerBridge`
...
3. **Host Counterpart**: The target-side stream is consumed on the PC host by
   `control-rs-ets-host::ServerBridge` (`../ets-host/ets-host-design.md`), which
...
graph TD
    ...
    subgraph "Host Environment"
        BRG["control-rs-ets-host (ServerBridge)"]
...
On the host side, `control-rs-ets-host::ServerBridge`
