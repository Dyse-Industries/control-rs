...
The ETS framework is structured as a dual-targeted system. TUI and CI are
host frontends; `ServerBridge` drives both. The target-side `Server` is ETS
on a physical board and virtual Embedded Test Server (virtual ETS) under QEMU.
...
1. **Host-Side (PC)**:
    ...
    - **TUI**: Interactive frontend (TUI → virtual ETS or TUI → ETS).
    - **CI**: Non-interactive frontend (CI → virtual ETS or CI → ETS); writes
...
├── control-rs-ets/     # Target-side server event loop, settings registry and profiling
├── control-rs-xtask/   # Host-side orchestration (TUI, CI, ServerBridge)
...
* **On-target Execution**: Run the ETS suite on a physical target (
  e.g., Teensy 4.0/4.1 or STM32 nucleo) using the `xtask` orchestrator. Verify
  that tests execute and report back in real-time.
* **Continuous Integration**: Verify that GitHub Actions can run CI → virtual
  ETS (QEMU) and CI → ETS (board), capturing telemetry and failing the
  pipeline if a test fails.
...
* **Test Discovery Dependency Tradeoff**: Replacing the custom
  `.ets_test_suites` mechanism with `linkme` (§5.4) trades already-working
...
* **TUI Distribution**: The design specifies the TUI as an `xtask`-internal
  ...
  `documentation/xtask/control-rs-ets-overview.md`, §4.3) to obtain or run it —
  e.g., as a standalone binary, a `cargo install`-able crate or bundled
  behind the `ets` feature. Undecided; deferred.
