...
| **A generic wrapper type (for example, `Operand<S>`) rather than a trait**                                                  | Forces wrapper construction per operand, whereas traits keep the abstraction at the bound where `Matrix` already names `S`.                                                                                                                                                                                                                                                                                                                                                | §5.2 (dimforge, 2026)                                                        |
| **A single shared `ld` parameter across `GEMM`'s three matrix operands**                                            | Operand leading dimensions are independent of shape and origin (for example, submatrix views), so different operands require different strides.                                                                                                                                                                                                                                                                                                                                    | §5.2 (Dongarra et al., 1990)                                                 |
