...
> for hardware-supported types (for example, `f32`, `Complex32`, `q31`, `q15`).
...
| **Omitting Complex / Hermitian BLAS**                                                                               | Prevents multi-input multi-output (MIMO) frequency domain analysis ($G(j\omega)$), $\mathcal{H}_\infty$ control, and quantum state estimation.                                                                                                                                                                                                                                                                                                                             | §1, §4.2, §4.3 (Dongarra et al., 1988, 1990; Netlib, 2026)                   |
...
| **Hardcoding one backend now (for example, CMSIS-DSP)**                                                                     | The interface's purpose is to remain library-agnostic and CMSIS-DSP does not target RISC-V32IMAC.                                                                                                                                                                                                                                                                                                                                                                          | §1, §4.5 (Arm Software, 2026a)                                               |
| **Runtime backend dispatch**                                                                                        | The backend is a type chosen at the call site (`DefaultBlas` or an example marker, §4.5), so runtime dispatch tests a statically known condition and adds execution overhead.                                                                                                                                                                                                                                                                                                                                       | §4.5 (ndarray, 2026)                                                         |
...
| **CONTIGUOUS-only slices, with `matrix` keeping loops for strided access**                                          | Roughly half of `matrix`'s inner loops read a fixed row of column-major storage (strided); leaving them outside the interface limits optimization.                                                                                                                                                                                                                                                                                                                         | §4.4 (Netlib, 2026)                                                          |
| **A gather/scatter view type instead of stride parameters**                                                         | Copying a strided row into a contiguous scratch buffer costs an $O(D)$ copy and stack allocation, violating the stack-only footprint.                                                                                                                                                                                                                                                                                                                                      | §4.4 (rust-embedded, 2026a)                                                  |
| **`INC_X`/`INC_Y`/`LDA`/`LDB`/`LDC` as compile-time const generics**                                                  | Strides are runtime `isize` values read from `r_stride()` / `c_stride()` (`storage-design.md` FR-2, §4.1), so one kernel serves submatrix, row and transposed views whose strides are not part of their type. Const-generic strides would multiply monomorphizations per view shape. The kernel reads each stride once at entry, which keeps the inner loop branchless (§4.4). | §4.1, §4.4 (Netlib, 2026)                                                    |
| **Reinterpreting `order: MatrixLayout` as a transpose flag**                                                      | Conflates memory layout with the algebraic operation. The `Trans` argument carries the operation, and zero-copy transposition is a strided view with swapped strides (`transpose_view`, `storage-design.md` FR-2; `matrix-design.md` §4.3.1), which the kernels consume like any other view. | §4.1 (Anderson et al., 1999)                                                 |
| **Leaving `GER`/`TRSV` to caller loops**                                                                            | They are precisely the operations accelerated by hardware; excluding them concedes performance on standard $O(D^2)$ loops.                                                                                                                                                                                                                                                                                                                                                 | §4.4 (Lawson et al., 1979; Netlib, 2026)                                     |
| **Hand-computing `lda`/`inc_x` at each call site**                                                                  | Pushes computation to call sites with no compiler checking that `lda` matches the layout/shape.                                                                                                                                                                                                                                                                                                                                                                            | §4.1 (Netlib, 2026)                                                        |
| **Two storage traits, one contiguous and one strided, bridged by a blanket impl**                                   | Violates Rust coherence rules (E0119) and lacks standard strided BLAS/LAPACK models.                                                                                                                                                                                                                                                                                                                                                                                       | `storage-design.md` §5 (dimforge, 2026)                                                  |
| **A generic wrapper type (for example, `Operand<S>`) rather than a trait**                                                  | Forces wrapper construction per operand, whereas traits keep the abstraction at the bound where `Matrix` already names `S`.                                                                                                                                                                                                                                                                                                                                                | §4.4 (dimforge, 2026)                                                        |
| **A single shared `ld` parameter across `GEMM`'s three matrix operands**                                            | Operand leading dimensions are independent of shape and origin (for example, submatrix views), so different operands require different strides.                                                                                                                                                                                                                                                                                                                                    | §4.4 (Dongarra et al., 1990)                                                 |
...
against analytical reference definitions using dimension-scaled and condition-scaled
...
- **Jacobi Budget as a Public Parameter**: $50 n^2$ is the
  ...
  bounded execution; promoting it to an associated constant (for example, `const
...
- **Enforcement of NFR-1a–NFR-1c (tracked)**: The sub-clauses (NFR-1a no `std`
  ...
  `cfg`-conditional public API) are exercised on the 4 virtual ETS (QEMU)
  targets of `ci-design.md` C-5 (`thumbv7em-none-eabihf`,
  `thumbv7em-none-eabi`, `riscv32imac-unknown-none-elf`,
  `riscv64gc-unknown-none-elf`) by the `ets` gate. Static AST-level lint
  checks are tracked in Phase 6.
...
| 1.5      | September 24, 2026 | @MitchellDScott | §5: const-generic strides and the `transpose_view` rejection corrected to the runtime-stride model; dangling §4.2.x / §5.x / §4.6 references retargeted; runtime-dispatch row states call-site backend choice. §8 ETS targets match `ci-design.md` C-5. |
