...
fn enable_dwt() {
    unsafe {
        ...
        demcr.write_volatile(demcr.read_volatile() | 0x0100_0000);
...
fn bench_cy(iters: u32, mut body: impl FnMut()) -> u32 {
    ...
    cyccnt().wrapping_sub(start) / iters.max(1)
}
...
fn report(
    ...
    hprintln!(
        "  {}: max diff = {:.2e}  backend={} cy/call  DefaultBlas={} cy/call  ({:.2}x) ... OK",
        label,
        diff,
        backend_cy,
        default_cy,
...
fn main() -> ! {
    enable_dwt();
    hprintln!("=== ARM Cortex-M7 CMSIS-DSP Subprogram Verification ===");
    hprintln!(
        "Shapes: L1 N={}, Gemv {}x{}, Gemm {}x{}x{}, Potrf/Trsm {}x{}.",
        N_L1,
        GV,
        GV,
        GM,
    ...
    hprintln!(
        "Times averaged over {}/{}/{} iters (L1/L2/L3). QEMU is not a speedup claim.",
        ITERS_L1,
        ITERS_L2,
        ITERS_L3
    );

    {
        ...
        let mut x_cmsis = ArrayStorage::<f32, N_L1, 1>::from_array([x_arr]);
        let mut x_ref = ArrayStorage::<f32, N_L1, 1>::from_array([x_arr]);
        let y_s = ArrayStorage::<f32, N_L1, 1>::from_array([y_arr]);

        CmsisDspBlas::scal(2.5, &mut x_cmsis);
        DefaultBlas::scal(2.5, &mut x_ref);
        let diff_scal = max_abs_diff(x_cmsis.as_slice(), x_ref.as_slice());
        let be = bench_cy(ITERS_L1, || {
            CmsisDspBlas::scal(2.5, &mut x_cmsis);
            black_box(x_cmsis.as_slice());
        });
        ...
        let diff_dot = libm::fabsf(dot_cmsis - dot_ref);
        let be = bench_cy(ITERS_L1, || {
            black_box(CmsisDspBlas::dotu(&x_dot, &y_s));
        });
        let re = bench_cy(ITERS_L1, || {
...

    {
        ...
        let diff_gemv = max_abs_diff(vy_cmsis.as_slice(), vy_ref.as_slice());
        ...
        let diff_gemv_t =
            max_abs_diff(vy_t_cmsis.as_slice(), vy_t_ref.as_slice());
        let be = bench_cy(ITERS_L2, || {
            CmsisDspBlas::gemv(
                Trans::Trans,
                1.5,
...

    {
        ...
        let diff_gemm = max_abs_diff(c_cmsis.as_slice(), c_ref.as_slice());
        ...
        let diff_gemm_t =
            max_abs_diff(c_t_cmsis.as_slice(), c_t_ref.as_slice());
        let be = bench_cy(ITERS_L3, || {
            CmsisDspBlas::gemm(
                Trans::Trans,
                Trans::NoTrans,
...

    {
        ...
        let res_cmsis = CmsisDspBlas::potrf(UpLo::Lower, &mut chol_cmsis);
        let res_ref = DefaultBlas::potrf(UpLo::Lower, &mut chol_ref);
        assert!(res_cmsis.is_ok() && res_ref.is_ok());
        let diff_chol =
            max_abs_diff(chol_cmsis.as_slice(), chol_ref.as_slice());
        let be = bench_cy(ITERS_L3, || {
            let mut tmp =
                RowArrayStorage::<f32, FACT, FACT>::from_array(spd_data);
            let _ = black_box(CmsisDspBlas::potrf(UpLo::Lower, &mut tmp));
        });
        let re = bench_cy(ITERS_L3, || {
            let mut tmp =
                RowArrayStorage::<f32, FACT, FACT>::from_array(spd_data);
        ...
        let trsm_cmsis = CmsisDspBlas::trsm(
            Side::Left,
            UpLo::Upper,
            Trans::NoTrans,
            Diag::NonUnit,
        ...
        assert!(trsm_cmsis.is_ok() && trsm_ref.is_ok());
        let diff_trsm = max_abs_diff(b_cmsis.as_slice(), b_ref.as_slice());
        let be = bench_cy(ITERS_L3, || {
            let mut tmp =
                RowArrayStorage::<f32, FACT, RHS_N>::from_array(rhs_data);
            let _ = black_box(CmsisDspBlas::trsm(
                Side::Left,
                UpLo::Upper,
                Trans::NoTrans,
                Diag::NonUnit,
...

    {
        ...
        let diff_strided =
            max_abs_diff(vy_sub_cmsis.as_slice(), vy_sub_ref.as_slice());
        let be = bench_cy(ITERS_L2, || {
            CmsisDspBlas::gemv(
                Trans::NoTrans,
                1.0,
            ...
            black_box(vy_sub_cmsis.as_slice());
        });
        let re = bench_cy(ITERS_L2, || {
            DefaultBlas::gemv(
                Trans::NoTrans,
