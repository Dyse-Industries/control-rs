...
fn main() {
    ...
    // Physical parameters
    let inertia = 0.01; // Rotor inertia J (kg*m^2)
    let friction = 0.1; // Viscous damping b (N*m*s)
    let kt = 0.01; // Torque constant (N*m/A)
    let ke = 0.01; // Back-EMF constant (V*s/rad)
    let resistance = 1.0; // Armature resistance R (Ohm)
    let inductance = 0.5; // Armature inductance L (H)
    ...
    let a = Owned::<f64, 2, 2>::from_row_arrays([
        [-friction / inertia, kt / inertia],
        [-ke / inductance, -resistance / inductance],
    ]);
    let b_mat =
        Owned::<f64, 2, 1>::from_row_arrays([[0.0], [1.0 / inductance]]);
    let c_mat = Owned::<f64, 1, 2>::from_row_arrays([[1.0, 0.0]]);
    let d_mat = Owned::<f64, 1, 1>::from_row_arrays([[0.0]]);

    let motor_ss = ArrayStateSpace::continuous(a, b_mat, c_mat, d_mat);
    print_controllability(&motor_ss);
    ...
    let den_0 = friction.mul_add(resistance, kt * ke);
    let den_1 = inertia.mul_add(resistance, friction * inductance);
    let den_2 = inertia * inductance;
    ...
    simulate_step(&motor_dt, sample_time);

    println!("\nSimulation completed successfully.");
}

/// Prints the controllability matrix `Mc = [ B | A*B ]` and its determinant.
fn print_controllability(motor_ss: &ArrayStateSpace<f64, 2, 1, 1>) {
    ...
    println!("  [{m00:8.4}, {m01:8.4}]");
    println!("  [{m10:8.4}, {m11:8.4}]");
...
/// Prints magnitude and phase of `motor_tf` at four frequencies.
fn print_bode(motor_tf: &ArrayTransferFunction<f64, 1, 3>) {
    ...
    for &omega in &frequencies {
        ...
        println!(
            "  w = {omega:6.1} rad/s -> Mag = {mag_db:7.2} dB, Phase = {phase_deg:7.2} deg"
        );
    }
}

/// Discrete-time step simulation: unit step voltage input (12V).
fn simulate_step(motor_dt: &ArrayStateSpace<f64, 2, 1, 1>, sample_time: f64) {
