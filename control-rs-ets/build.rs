...
/// Build-script outcome; any I/O or environment error fails the build.
type BuildResult = Result<(), Box<dyn Error>>;

fn main() -> BuildResult {
    let out_dir = PathBuf::from(env::var("OUT_DIR")?);

    // Custom Linker Section for ETS Test Suites
    let mut ets_file = File::create(out_dir.join("ets_suites.x"))?;
    ets_file.write_all(
        b"
SECTIONS
{
