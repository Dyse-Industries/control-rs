...
/// HIL and unit test suite for matrices.
pub mod matrix_tests {
    ...
    fn test_matrix_addition_and_subtraction() {
        ...
        // Add / Sub by value
        let m3 = m1 + m2;
        assert_eq!(m3.get(0, 0), Some(&11));
        assert_eq!(m3.get(1, 1), Some(&44));

        let m4 = m3 - m2;
        assert_eq!(m4.get(0, 0), Some(&1));
        assert_eq!(m4.get(1, 1), Some(&4));
    }
    ...
    fn test_matrix_scaling() {
        ...
        let m2 = m * 3.0;
        assert_eq!(m2.get(0, 0), Some(&3.0));
        assert_eq!(m2.get(1, 1), Some(&12.0));

        let m3 = m2 / 3.0;
        assert_eq!(m3.get(0, 0), Some(&1.0));
        assert_eq!(m3.get(1, 1), Some(&4.0));
    }
    ...
    fn test_matrix_multiplication() {
        ...
        // [ 8.0,  10.0 ]
        // [ 1.0,  2.0 ]
        let b = Matrix::new([[7.0, 8.0, 1.0], [9.0, 10.0, 2.0]]);

        // C = A * B -> 2 x 2
        // C[0, 0] = 1*7 + 3*8 + 5*1 = 7 + 24 + 5 = 36
        // C[1, 0] = 2*7 + 4*8 + 6*1 = 14 + 32 + 6 = 52
        // C[0, 1] = 1*9 + 3*10 + 5*2 = 9 + 30 + 10 = 49
        // C[1, 1] = 2*9 + 4*10 + 6*2 = 18 + 40 + 12 = 70
        let c = a * b;
        assert_eq!(c.rows(), 2);
        assert_eq!(c.cols(), 2);
        assert_eq!(c.get(0, 0), Some(&36.0));
        assert_eq!(c.get(1, 0), Some(&52.0));
        assert_eq!(c.get(0, 1), Some(&49.0));
        assert_eq!(c.get(1, 1), Some(&70.0));
    }
    ...
    fn test_symmetric() {
        let m =
            Matrix::new([[1.0, 0.0, 0.0], [0.0, 2.0, 0.0], [0.0, 0.0, 3.0]]);
