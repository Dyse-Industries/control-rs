use crate::math::dsp::{Convolution, FFT};

#[cfg_attr(not(test), control_rs_macros::hil_suite)]
/// Advanced Digital Signal Processing (DSP) algorithm tests for FFT and Convolution.
pub mod dsp_advanced {
    use super::*;
    #[cfg(not(test))]
    use crate::math::num_traits::Trig;
    use crate::math::{Bijection, Map, complex_num::Complex};

    const N: usize = 8;
    const TOLERANCE: f64 = 1e-6;

    #[cfg_attr(test, test)]
    fn test_dsp_fft_impulse_advanced() {
        let mut input = [0.0f64; N];
        input[0] = 1.0;
        let mut output = [Complex::default(); N];

        TestFFT::fft(&input, &mut output);

        for val in output {
            assert!((val.re - 1.0f64).abs() < TOLERANCE);
            assert!(val.im.abs() < TOLERANCE);
        }
    }

    #[cfg_attr(test, test)]
    fn test_dsp_fft_dc_advanced() {
        let input = [1.0; N];
        let mut output = [Complex::default(); N];

        TestFFT::fft(&input, &mut output);

        assert!(
            (output[0].re - f64::from(u32::try_from(N).unwrap())).abs()
                < TOLERANCE
        );
        assert!(output[0].im.abs() < TOLERANCE);

        for val in output.iter().take(N).skip(1) {
            assert!(val.re.abs() < TOLERANCE);
            assert!(val.im.abs() < TOLERANCE);
        }
    }

    #[cfg_attr(test, test)]
    fn test_dsp_fft_ifft_identity_advanced() {
        let mut input = [0.0f64; N];
        for (i, val) in input.iter_mut().enumerate() {
            *val = f64::from(u32::try_from(i).unwrap()).sin();
        }

        let mut fft_output = [Complex::default(); N];
        TestFFT::fft(&input, &mut fft_output);

        let mut ifft_output = [0.0f64; N];
        TestFFT::ifft(&fft_output, &mut ifft_output);

        for (in_val, out_val) in input.iter().zip(ifft_output.iter()) {
            assert!((in_val - out_val).abs() < TOLERANCE);
        }
    }

    #[cfg_attr(test, test)]
    fn test_dsp_map_bijection_traits_advanced() {
        let time_signal = [1.0, 0.0, 0.0, 0.0];
        let fft = TestFFT;
        let freq_signal = fft.evaluate(time_signal);
        let recovered: [f64; 4] = fft.evaluate_inverse(freq_signal);
        assert!((recovered[0] - 1.0_f64).abs() < 1e-6);
    }

    #[cfg_attr(test, test)]
    fn test_dsp_convolution_identity_advanced() {
        let input = [1.0, 2.0, 3.0];
        let kernel = [1.0];
        let mut output = [0.0f64; 3];
        TestConvolution::convolve_input(&input, &kernel, &mut output);
        for (out_val, in_val) in output.iter().zip(input.iter()) {
            assert!((out_val - in_val).abs() < TOLERANCE);
        }
    }

    #[cfg_attr(test, test)]
    fn test_dsp_convolution_with_impulse_advanced() {
        let input = [1.0, 2.0, 3.0];
        let kernel = [0.0, 1.0, 0.0];
        let mut output = [0.0f64; 5];
        TestConvolution::convolve_input(&input, &kernel, &mut output);
        let expected = [0.0, 1.0, 2.0, 3.0, 0.0];
        for (out_val, exp_val) in output.iter().zip(expected.iter()) {
            assert!((out_val - exp_val).abs() < TOLERANCE);
        }
    }

    #[cfg_attr(test, test)]
    fn test_dsp_convolution_boxcar_advanced() {
        let input = [1.0, 1.0];
        let kernel = [1.0, 1.0];
        let mut output = [0.0f64; 3];
        TestConvolution::convolve_input(&input, &kernel, &mut output);
        let expected = [1.0, 2.0, 1.0];
        for (out_val, exp_val) in output.iter().zip(expected.iter()) {
            assert!((out_val - exp_val).abs() < TOLERANCE);
        }
    }

    #[cfg(test)]
    #[test]
    #[should_panic(expected = "Convolution output buffer is too small")]
    fn _test_dsp_convolve_buffer_panic_advanced() {
        let input = [1.0, 2.0];
        let kernel = [1.0, 1.0];
        let mut output = [0.0; 1];
        TestConvolution::convolve_input(&input, &kernel, &mut output);
    }

    #[cfg_attr(test, test)]
    fn test_dsp_convolution_empty_advanced() {
        let input = [];
        let kernel = [1.0];
        let mut output = [];
        TestConvolution::convolve_input(&input, &kernel, &mut output);

        let input2 = [1.0];
        let kernel2 = [];
        let mut output2 = [];
        TestConvolution::convolve_input(&input2, &kernel2, &mut output2);
    }
}

struct TestConvolution;
struct TestFFT;

impl<T: crate::math::num_traits::Real> Convolution<T> for TestConvolution {}

impl<
    T: 'static
        + Copy
        + crate::math::num_traits::Real
        + crate::math::ops::Neg<Output = T>
        + Default,
> FFT<T> for TestFFT
{
}
