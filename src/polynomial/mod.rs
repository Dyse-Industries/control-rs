...
use crate::math::storage::{
    ArrayStorage, ContiguousStorage, ContiguousStorageMut, Storage, StorageMut,
    StorageView, StorageViewMut,
};
use crate::math::subprograms::{DefaultBlas, level1::Axpy};
use crate::math::{ArithmeticError, ConversionError, ConversionResult};
    ...
    Const<N>: Dim,
{
    ...
    {
        Self {
            storage: ArrayStorage::from_array([[T::ZERO; N]; 1]),
            _marker: PhantomData,
        }
    }
    ...
    where
        T: Copy,
    {
        let mut storage_data = [[data[0]; N]; 1];
        let mut i = 0;
        while i < N {
            storage_data[0][i] = data[i];
            i += 1;
        }
        Self {
            storage: ArrayStorage::from_array(storage_data),
            _marker: PhantomData,
        }
    }
    ...
    Const<N>: Dim,
{
    ...
    /// Multiplies two polynomials returning an exact degree-bounded polynomial using a specific DSP engine.
    ///
    /// Product capacity $P = N + M - 1$.
    pub fn mul_poly_with<DSP, const M: usize, const P: usize>(
        &self,
    ...
    /// Multiplies two polynomials returning an exact degree-bounded polynomial using the default DSP engine.
    ///
    /// Product capacity $P = N + M - 1$.
    ...
    Const<N>: Dim,
{
    ...
    ///
    /// # Errors
    /// Returns `ArithmeticError::DivisionByZero` if divisor is zero polynomial.
    pub fn div_rem<const M: usize, const Q: usize, const R: usize>(
        &self,
        divisor: &ArrayPolynomial<T, M>,
    ) -> Result<(ArrayPolynomial<T, Q>, ArrayPolynomial<T, R>), ArithmeticError>
    where
        T: Float,
        Const<M>: Dim,
        Const<Q>: Dim,
        Const<R>: Dim,
    {
        ...
        let deg_b = divisor.degree().ok_or(ArithmeticError::DivisionByZero)?;
