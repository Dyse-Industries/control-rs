...
use crate::math::{
    complex_num::Complex,
    num_traits::{One, Real, Zero},
    ops::{AddAssign, Div, Mul, Sub},
    storage::arrays_from_zipped_iterator,
};

type ComplexMatrix<T, const N: usize, const M: usize, const K: usize> =
    [[[Complex<T>; K]; M]; N];
type PhaseGainMatrix<T, const N: usize, const M: usize> =
    [[PhaseGainCrossover<T>; N]; M];
type FloatArray<T, const K: usize> = [T; K];
type FloatTupleArray<T, const K: usize> = (FloatArray<T, K>, FloatArray<T, K>);
type ComplexArray<T, const K: usize> = [Complex<T>; K];
type MarginTuple<T> = (Option<T>, Option<T>);
type CrossoverTuple = (usize, bool);

/// A trait for types that can represent an angle and provide
/// conversion from degrees to radians.
pub trait Phase: Real {
...
pub trait Magnitude: Real {
    ...
    ///
    /// The formula used is `20 * log10(self)`.
    #[must_use]
    #[inline]
    fn to_db(self) -> Self {
    ...
    ///
    /// The formula used is `10^(self / 20)`.
    #[must_use]
    #[inline]
    fn to_mag(self) -> Self {
        Self::TEN.pow(self / Self::TWENTY)
    }
}
...
pub trait FrequencyTools<T, const N: usize, const M: usize> {
    ...
    fn frequency_response<const K: usize>(
        &self,
        response: &mut FrequencyResponse<T, N, M, K>,
    );
}
...
pub struct FrequencyResponse<T, const N: usize, const M: usize, const K: usize>
{
    ...
    pub responses: Option<ComplexMatrix<T, N, M, K>>,
}
...
pub struct FrequencyMargin<T, const N: usize, const M: usize>(
    pub PhaseGainMatrix<T, N, M>,
);
...
impl<T, const N: usize, const M: usize, const K: usize>
    FrequencyResponse<T, N, M, K>
{
    ...
    T: Real,
{
    ...
    ) -> Option<FloatTupleArray<T, K>> {
        self.responses
            .as_ref()
            .and_then(|responses| responses.get(input_channel))
            .and_then(|row| row.get(output_channel))
            .map(|response| unzip_complex_array(response))
    }
}
...
impl<T: Copy + Zero + One + Real + Phase + Magnitude> PhaseGainCrossover<T> {
    ...
    pub fn new<const K: usize>(
        frequencies: &[T; K],
        response: &ComplexArray<T, K>,
    ) -> Self {
        let (magnitudes, phases) = unzip_complex_array(response);
        let (gain_margin, phase_crossover) =
            find_margin(&phases, &magnitudes, frequencies, -T::PI);
...
impl<
    T: Copy + Zero + One + Real + Magnitude + Phase,
    const N: usize,
    const M: usize,
> FrequencyMargin<T, N, M>
{
    ...
    pub fn new<const K: usize>(
        frequency_response: &FrequencyResponse<T, N, M, K>,
    ) -> Self {
        let mut margins = [[PhaseGainCrossover::default(); N]; M];
        if let Some(responses) = &frequency_response.responses {
            for (margin_row, response_row) in
                margins.iter_mut().zip(responses.iter())
            {
                for (margin, response) in
                    margin_row.iter_mut().zip(response_row.iter())
                {
                    *margin = PhaseGainCrossover::new(
...
pub fn unzip_complex_array<T: Real, const K: usize>(
    complex_array: &ComplexArray<T, K>,
) -> FloatTupleArray<T, K> {
...
{
    if let Some((index, is_exact)) =
        first_crossover_index(crossover, &threshold)
    {
        if is_exact {
            (Some(a[index].clone()), Some(b[index].clone()))
        } else {
            (
                Some(array_interpolation(
                    crossover,
                    a,
                    index,
                    threshold.clone(),
...
#[allow(clippy::indexing_slicing)]
fn first_crossover_index<T, const K: usize>(
    array: &[T; K],
    threshold: &T,
) -> Option<CrossoverTuple>
where
    T: PartialOrd,
{
    for i in 0..K - 1 {
...
#[allow(clippy::indexing_slicing)]
fn array_interpolation<T, const K: usize>(
    a: &[T; K],
    b: &[T; K],
    index: usize,
    threshold: T,
    ...
    T: Clone + PartialOrd + Sub<Output = T> + Mul<Output = T> + Div<Output = T>,
{
    ...
    // and c = y1 - m*x1
    // We want x = (y - c) / m
    if a[index].clone() == a[index + 1].clone()
        || b[index].clone() == b[index + 1].clone()
    {
        return b[index].clone();
    }

    // Calculate the slope of 'a' with respect to 'b'
    let slope = (a[index + 1].clone() - a[index].clone())
        / (b[index + 1].clone() - b[index].clone());
...
/// # Example
/// ```
/// use control_rs::classical_tools::frequency_tools::logspace;
/// let points = logspace::<f64, 5>(1.0, 3.0);
/// assert_eq!(points.len(), 5);
/// ```
pub fn logspace<T: Real + AddAssign, const N: usize>(a: T, b: T) -> [T; N] {
    ...
    let step = (b - a.clone()) / T::from_usize(N - 1);

    let mut exponent = a;
    for r in &mut result {
        *r = ten.clone().pow(exponent.clone());
        exponent += step.clone();
    }
...
mod test_log_space {
    ...
    fn basic() {
        ...
        assert_almost_eq!(ls[0], 10.0);
        assert_almost_eq!(ls[1], 27.825594);
        assert_almost_eq!(ls[2], 77.42636);
        assert_almost_eq!(ls[3], 215.44342);
        assert_almost_eq!(ls[4], 599.48413);
        assert_almost_eq!(ls[5], 1668.1001);
        assert_almost_eq!(ls[6], 4641.587);
        assert_almost_eq!(ls[7], 12915.498);
        assert_almost_eq!(ls[8], 35938.16);
        assert_almost_eq!(ls[9], 100_000.11);
    }
}
...
mod test_find_margin {
    ...
    fn crossover_detection() {
        ...
        assert_almost_eq!(phase_margin, -f64::PI);

        assert!(margin.1.is_some());
        #[allow(clippy::unwrap_used)]
        let gain_crossover = margin.1.unwrap();
        assert_almost_eq!(gain_crossover, 1.0);
    }
    ...
    fn crossover_interpolation() {
        ...
        #[allow(clippy::unwrap_used)]
        let gain_crossover = margin.1.unwrap();
        assert_almost_eq!(gain_crossover, 1.25);
    }
    ...
    fn decreasing_crossover() {
        ...
        assert!(!index.1);
        let result =
            array_interpolation(&magnitudes, &frequencies, index.0, threshold);
        assert_almost_eq!(result, 1.25); // Interpolated value
    }
    #[allow(clippy::unwrap_used)]
    #[test]
    ...
    fn integer_interpolation() {
        ...
        let magnitudes: [i32; 10] =
            core::array::from_fn(|i| if i < 5 { 5 } else { 7 });
        let threshold = 6;
        let index = first_crossover_index(&magnitudes, &threshold).unwrap();
        assert!(!index.1);
    ...
    fn multiple_crossovers() {
        ...
        let magnitudes: [f32; 10] =
            core::array::from_fn(|i| if i % 2 == 0 { 1.0 } else { 3.0 });
        let threshold = 2.0;
        let index = first_crossover_index(&magnitudes, &threshold).unwrap();
        assert!(!index.1);
        let result =
            array_interpolation(&magnitudes, &frequencies, index.0, threshold);
        assert_almost_eq!(result, 0.5);
    }
    #[test]
