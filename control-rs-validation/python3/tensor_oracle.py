#!/usr/bin/env python3
"""
python3/tensor_oracle.py

Executes NumPy/SciPy/TFLite/ONNX Runtime equivalents for tensor models.
Writes `results/tensor.scipy.h5`, `results/tensor.numpy.h5`, and `results/tensor.tflite.h5`.
"""
...
from h5_write import write_dict_to_h5
...
if __name__ == "__main__":
    ...
    scipy_payload = {
        ...
        "contraction": results["contraction"],
        "boundaries": {
            "q_raw": boundaries["q_raw"],
            "act_outputs": boundaries["act_exact"],
            "act_inputs": boundaries["act_inputs"],
        },
        "timing": results["timing"],
    }
    numpy_payload = {"boundaries": {"q_raw": boundaries["q_raw"]}}
    tflite_payload = {
        "boundaries": {"act_outputs": boundaries["tflite_dequant"]}
    }
    ...
    signal_keys = {
        "manifold/interp_mesh": "nmv.tensor.manifold.interp_mesh",
        "contraction/mat_c": "nmv.tensor.contraction.mat_c",
        "boundaries/q_raw": "nmv.tensor.boundaries.q_raw",
        "boundaries/act_outputs": "nmv.tensor.boundaries.act_outputs",
    }
    table = Path(__file__).resolve().parent.parent / "tolerances/numerical_models.toml"
    specs = attr_specs_from_toml(
        table,
        signal_keys,
        {"tflite": {"boundaries/act_outputs": "nmv.tensor.boundaries.tflite_dequant"}},
    )
    out = Path("results")
    write_variant_file(
        out / "tensor.scipy.h5",
